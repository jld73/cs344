...
def load_val(sliders, encoder, X_test):
    vals = encoder.predict(X_test)
    to_load = random.choice(vals)
    print(to_load)
...
tk.Button(g, text="Load Image", command=lambda: load_val(sliders, encoder, X_test)).grid(row=5,column=1)
