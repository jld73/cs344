...
import os
# load images
traindir = r"C:\Users\Jordan\Downloads\BizHawk\SNES\Screenshots\train"
testdir = r"C:\Users\Jordan\Downloads\BizHawk\SNES\Screenshots\test"
traindirectory = os.fsencode(traindir)
testdirectory = os.fsencode(testdir)
test = 0
val = 0
train = 0
crop = 50
base_height = 224
height = base_height-crop
width = 256
...
for file in os.listdir(traindirectory):
    ...
    #print(np.array(Image.open(os.path.join(traindir, filename)).convert('L')).shape)
    
    X_train.append(np.array(Image.open(os.path.join(traindir, filename)).convert('L').crop(box=(0,crop,width,base_height))).reshape(height*width,))
    i+=1
...
for file in os.listdir(testdirectory):
    ...
    X_test.append(np.array(Image.open(os.path.join(testdir, filename)).convert('L').crop(box=(0,crop,width,base_height))).reshape(height*width,))
    i+=1
...
# plt.imshow(X_test[0].reshape(height,width))
# plt.show()
# sys.exit()
#print(X_test[0])
...
decoded = Dense(units=height*width, activation='sigmoid')(decoded)
...
generator = Dense(units=512, activation='relu')(invector)
generator = Dense(units=2048, activation='relu')(generator)
generator = Dense(units=height*width, activation='sigmoid')(generator)
...
def render(sliders, gui, lbl, ex):
    ...
    bip *= 255
    img = Image.fromarray(bip.reshape(height,width))
    ...
    gui.after(50, render, sliders, gui, lbl, ex)
...
def load_val(sliders, encoder, X_test, gui, ex):
    ...
    gui.ex_image = tki.PhotoImage(Image.fromarray(orig.reshape(height,width)).resize((width,height), Image.ANTIALIAS))
...
l2 = tk.Label(g, image=g.disp_image)
l2.grid(row= 9, column=4)
tk.Button(g, text="Load Image", command=lambda: load_val(sliders, encoder, X_test, g, l2)).grid(row=9,column=1)
g.after(50, render, sliders, g, l, l2)
